...
#headers = {"Authorization": "Bearer ya29.a0ARrdaM_vbg_VrxE2i8aV8F6pQf7E1uPD_FDMF2yvW5-Vp-xZPlvBozkL1Mr6fKSic8g38WAFUDSDjSDvStrWO3cZL0nDiU61mrrhg9oqe0qzqNzZVp0fkrxRCw25bVIgj3wu6fxhkFeK66Kla6yyDmw-I2ic"}
...
def checkdisease(request):
   if request.method == 'GET':
      ...
      puser = User.objects.get(username=patientusername)
      now = datetime.now()
      now_str = now.strftime("%d-%m-%Y_%Hh%M")
      filename = str(puser)+ "_" + now_str +"_"+ uploaded_file.name
      fs = FileSystemStorage()
      file = fs.save(filename,uploaded_file)
      '''
      #document = FilesUpload.objects.create(file=uploaded_file)
      path = './media/' + uploaded_file.name
 
      gfile = drive.CreateFile({'parents': [{'id': '18F8HdlLIxDgN7rrgtFcyg7fUpQa3JH3Q'}]})
      # Read file and set it as the content of this instance.
      gfile.SetContentFile(path)
      gfile["title"] = uploaded_file.name
      gfile.Upload() # Upload the file.
      #document.save()

      #test = pd.read_csv(uploaded_file,skiprows=21)
      return render(request, 'patient/checkdisease/checkdisease.html')
      '''
      uploaded_file_path = fs.path(file)
      df = pd.read_csv('./data/Database Tiêu xay mới.csv')
      df['Code_dialy'] = df['Code'].map(lambda x: my_dict.get(x) if x in my_dict else x)
         #df['Code_loai_mau'] = df['Code'].map(lambda x: my_dict.get(x) if x in my_dict else x)
      le= LabelEncoder()
      y_0 = df['Code_dialy']
      ...
      test = pd.read_csv(uploaded_file_path,skiprows=21)
      loaded_model = pickle.load(open('./data/model_dialy_tieuxay.yml', 'rb'))
      test = test.iloc[:,1]
      #absoulute = np.asarray(test).T
      test = np.asarray(test).reshape(1,-1)
      result = loaded_model.predict_proba(test)
      result2 = ((np.sort(result)).T)*100
      result = (result.T)*100

      a = np.where(result == result2[-1])
      id1 = le_name_mapping[a[0][0]]
      KQ1 =  "Dự đoán 1: Mẫu " + str(id1) + ", Tỷ lệ chính xác: " + str(int(result2[-1])) + '%'
      a = np.where(result == result2[-2])
      id2 = le_name_mapping[a[0][0]]

      KQ2 =  "Dự đoán 2: Mẫu " + str(id2) + ", Tỷ lệ chính xác: " + str(int(result2[-2])) + '%'
      return render(request, 'patient/checkdisease/checkdisease.html', {'KQ1':KQ1, 'KQ2':KQ2})
